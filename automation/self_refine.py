...
MIN_WEEKS   = 3    # don't refine until we have at least 3 weeks of data


# ── ANALYSIS ──────────────────────────────────────────────────────────────────

def information_coefficient(scores: pd.Series, returns: pd.Series) -> float:
    """Rank IC between scores and forward returns."""
...
def hit_rate(scores: pd.Series, returns: pd.Series, top_pct: float = 0.20) -> float:
    """% of top-scored tickers that outperformed the median return."""
    mask  = scores.notna() & returns.notna()
    if mask.sum() < 20:
        return float("nan")
...
def compute_attribution(log_df: pd.DataFrame) -> dict:
    """
    Per-signal IC and hit rate, broken down by regime.
    Returns nested dict: {signal: {overall: ic, by_regime: {regime: ic}}}
    """
    ...
    for sig in signals:
        if sig not in log_df.columns:
            continue
        ic_overall = information_coefficient(
            log_df[sig], log_df["forward_return_1w"])
        hr_overall = hit_rate(
            log_df[sig], log_df["forward_return_1w"])
...
def conviction_performance(log_df: pd.DataFrame) -> dict:
    """How did each conviction tier actually perform?"""
    out = {}
...
def weekly_alpha(log_df: pd.DataFrame) -> list[dict]:
    """Top-quintile score return vs universe average by week."""
    weeks = []
...
def build_analysis_prompt(
    attribution: dict,
    conviction_perf: dict,
    weekly: list[dict],
    current_weights: dict,
    n_weeks: int,
) -> str:
    lines = [
        f"PERFORMANCE ANALYSIS — last {n_weeks} weeks",
        "",
        "SIGNAL ATTRIBUTION (IC = rank correlation with forward returns):",
    ]
    ...
    for conv in ["very_high","high","elevated","moderate","low"]:
        if conv in conviction_perf:
            ...
            lines.append(f"  {conv:<12} mean={d['mean_return']:+.4f}  "
                         f"hit_rate={d['hit_rate']}  n={d['n']}")
    ...
    for w in weekly[-8:]:  # last 8 weeks
        ...
        lines.append(f"  {w['week']}  {w['regime']:<20} "
                     f"top_q={w['top_q_ret']}  avg={w['avg_ret']}  alpha={alpha_str}")
...
def validate_weights(new_w: dict, current_w: dict) -> tuple[bool, str]:
    """Validate new weights against schema constraints."""
    try:
        constraints = current_w.get("constraints", {})
        w_min = constraints.get("signal_weight_min", 0.05)
        w_max = constraints.get("signal_weight_max", 0.60)
        m_min = constraints.get("regime_multiplier_min", 0.10)
        m_max = constraints.get("regime_multiplier_max", 2.00)
        ...
        # Check no new keys added in signal_weights
...
def diff_weights(old: dict, new: dict) -> list[str]:
    """Human-readable diff of weight changes."""
    changes = []
    for sig, old_v in old.get("signal_weights", {}).items():
        ...
        if abs(new_v - old_v) > 0.001:
            ...
            changes.append(f"  signal_weights.{sig}: {old_v:.3f} → {new_v:.3f} {arrow}")
    ...
    for regime in old.get("regime_multipliers", {}):
        for sig, old_v in old["regime_multipliers"][regime].items():
            ...
            if abs(new_v - old_v) > 0.01:
                arrow = "↑" if new_v > old_v else "↓"
                changes.append(f"  regime_multipliers.{regime}.{sig}: {old_v:.2f} → {new_v:.2f} {arrow}")
...
def run():
    ...
    if not valid:
        ...
        # Write a note about the failed attempt
        note_path = REFINEMENTS / f"{date_str}.md"
        with open(note_path, "w") as f:
            f.write(f"# Refinement {date_str} — REJECTED\n\n")
            f.write(f"**Reason:** {reason}\n\n")
            f.write(f"Weeks analyzed: {n_weeks}\n")
        return

    # Preserve metadata fields from current
    new_weights["_meta"] = {
    ...
    note_lines = [
        f"# Refinement — {date_str}",
        "",
        f"**Weeks analyzed:** {n_weeks}",
        f"**Data points:** {len(log_df):,}",
        "",
        "## Changes",
        "",
    ]
    if changes:
        ...
        note_lines.append("No changes — current weights performing well.")
    ...
    for sig, data in attribution.items():
        note_lines.append(
            f"- **{sig}**: IC={data['ic_overall']}  hit_rate={data['hr_overall']}")

    note_lines += ["", "## Weekly Alpha (last 4 weeks)", ""]
    for w in weekly[-4:]:
        alpha_str = f"{w['alpha']:+.4f}" if w['alpha'] is not None else "n/a"
        note_lines.append(
            f"- {w['week']} `{w['regime']}` — alpha vs universe: {alpha_str}")
